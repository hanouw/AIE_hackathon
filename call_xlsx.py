...
df_filtered_과목종별_전기 = df[~df['평가'].isin(['W', 'NP', 'F', 'U']) & (df['과목종별'] == '전기')]
df_filtered_과목종별_전선 = df[~df['평가'].isin(['W', 'NP', 'F', 'U']) & (df['과목종별'] == '전선')]
df_filtered_과목종별_전필 = df[~df['평가'].isin(['W', 'NP', 'F', 'U']) & (df['과목종별'] == '전필')]
df_filtered_과목종별_RC = df[~df['평가'].isin(['W', 'NP', 'F', 'U']) & (df['과목종별'] == 'RC')]
df_filtered_과목종별_GLC교양 = df[~df['평가'].isin(['W', 'NP', 'F', 'U']) & df['과목종별'] == '대교' and df['학정번호'].str[:3] == 'GLC']
...
completed_credits = df_filtered_과목종별_전기.groupby("학점").sum()
